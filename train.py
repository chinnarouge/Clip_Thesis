...
class CXRDataset(data.Dataset):
    ...
    def __init__(self, img_path, txt_path, label_path, column='report', size=None, transform=None):
        ...
        if size != None: 
            ...
            self.label_dset = pd.read_csv(label_path)[['Adenopathy', 'Atelectasis', 'Azygos Lobe', 
                'Calcification of the Aorta', 'Cardiomegaly', 'Clavicle Fracture', 'Consolidation', 
                'Edema', 'Emphysema', 'Enlarged Cardiomediastinum', 'Fibrosis', 'Fissure', 'Fracture', 
                'Granuloma', 'Hernia', 'Hydropneumothorax', 'Infarction', 'Infiltration', 'Kyphosis', 
                ...
                'Subcutaneous Emphysema', 'Support Devices', 'Tortuous Aorta', 'Tuberculosis']][:size]
        else: 
...
def load_data(cxr_filepath, txt_filepath,label_filepath, batch_size=4, column='report', pretrained=False, verbose=False, type='train'): 
    ...
    torch_dset = CXRDataset(img_path=cxr_filepath,
                        txt_path=txt_filepath,label_path=label_filepath, column=column, transform=transform)
